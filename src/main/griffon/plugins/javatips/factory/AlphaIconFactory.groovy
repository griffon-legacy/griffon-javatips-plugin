...
import com.wordpress.tipsforjava.swing.AlphaIcon
 ...
 */
class AlphaIconFactory extends AbstractFactory {
    Object newInstance(FactoryBuilderSupport builder, Object name, Object value, Map attributes) {
	    float alpha = toFloat(attributes.remove('alpha'), 1.0f)
	
        if(value instanceof Icon) {
	        return new AlphaIcon(value, alpha)
        } else if(attributes.containsKey('icon')) {
	        return new AlphaIcon(attributes.remove('icon'), alpha)
        }
    }
}